            ...
            switch (call.method) {
                ...
                case "recognizer.create": {
                    ...
                    try {
                        ...
                        Object rawGrammar = argsMap.get("grammar");

                        if (rawGrammar instanceof List<?>) {
                            @SuppressWarnings("unchecked")
                            List<String> grammarList = (List<String>) rawGrammar;
                            JSONArray grammarJson = new JSONArray(grammarList);
                            recognizer = new Recognizer(model, sampleRate, grammarJson);
                            android.util.Log.d("PluginJava", "✅ Recognizer created with JSONArray grammar (List<String>)");
                        } else if (rawGrammar instanceof String) {
                            recognizer = new Recognizer(model, sampleRate, (String) rawGrammar);
                            android.util.Log.d("PluginJava", "✅ Recognizer created with String grammar");
                        } else {
                            recognizer = new Recognizer(model, sampleRate);
                            android.util.Log.d("PluginJava", "✅ Recognizer created without grammar");
                        }
                ...
                case "recognizer.setGrammar": {
                    Map<String, Object> argsMap = castMethodCallArgs(call, argsMapClass);
                    Integer recognizerId = getRequiredArgumentFromMap(argsMap, "recognizerId", Integer.class);
                    Object rawGrammar = getRequiredArgumentFromMap(argsMap, "grammar", Object.class);
                    ...
                    try {
                        if (rawGrammar instanceof List<?>) {
                            ...
                            android.util.Log.d("PluginJava", "✅ setGrammar(List<String>) applied via JSONArray");
                        } else if (rawGrammar instanceof String) {
                            recognizer.setGrammar((String) rawGrammar);
                            android.util.Log.d("PluginJava", "✅ setGrammar(String) applied directly");
                        } else {
                            result.error("INVALID_TYPE", "Grammar must be either String or List<String>", null);
                            return;
                        }
                        result.success(null);
                    } catch (Exception e) {
                        result.error("SET_GRAMMAR_ERROR", e.getMessage(), null);
                    }
                }
                break;
